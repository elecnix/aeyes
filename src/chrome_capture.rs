...
fn cdp_send(ws: &mut tungstenite::WebSocket<tungstenite::stream::MaybeTlsStream<std::net::TcpStream>>, method: &str, params: Value) -> Result<Value> {
...
pub fn capture_screenshot(quality: u32) -> Result<Vec<u8>> {
    ...
    let target_id = page["targetId"]
        .as_str()
        .context("targetId not a string")?;
    
    // 4. Attach to target
    let attach_response = cdp_send(&mut ws, "Target.attachToTarget", json!({
        "targetId": target_id,
        "flatten": true
    }))?;
